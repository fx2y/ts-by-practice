...
const getJobsHandler = rest.get(
  `${API_URL}/jobs`, async (req, res, ctx) => {
    const organizationId = req.url.searchParams.get("organizationId") as string;
    const jobs = db.job.findMany({ where: { organizationId: { equals: organizationId } } });
    return res(ctx.delay(300), ctx.status(200), ctx.json(jobs));
  }
);

const getJobHandler = rest.get(
  `${API_URL}/jobs/:jobId`, async (req, res, ctx) => {
    const jobId = req.params.jobId as string;
    const job = db.job.findFirst({ where: { id: { equals: jobId } } });
    if (!job) {
...
export const jobHandlers = [getJobsHandler, getJobHandler, createJobHandler];