...
const delayedFn = <T, A extends any[]>(fn: (...args: A) => T, delay: number) => (...args: A) => new Promise<T>((resolve) => setTimeout(() => resolve(fn(...args)), delay));
...
export const getOrganization = delayedFn(
  (id: string) => testData.organizations.find((org) => org.id === id) || null,
  300
);

export const getJobs = delayedFn(
  (organizationId: string) => testData.jobs.filter((job) => job.organizationId === organizationId),
  300
);

export const getJob = delayedFn(
  (id: string) => testData.jobs.find((job) => job.id === id) || null,
  300
);
...
export const useJobs = (organizationId: string) => useTestData(getJobs(organizationId));
